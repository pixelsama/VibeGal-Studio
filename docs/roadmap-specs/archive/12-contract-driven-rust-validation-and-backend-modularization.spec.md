...
### 10.1 实施后的 Requirement-to-Test 证据

| 需求 | 最终可执行证据 |
| --- | --- |
| C-01 / V-01 / V-02 / V-03 / V-05 / V-09 | `packages/contracts/src/validation.test.ts`、`packages/contracts/fixtures/validation-contract.json`；Engine、Rust、CLI 直接消费同一 corpus |
| C-02 | `packages/studio/src-tauri/tests/contract_artifact_provenance.rs` 与 `build_support/contracts.rs` 的 source/artifact hash fail-closed 测试 |
| C-03 | `pnpm run check:schemas` 同时比较 contracts 生成物、Rust embedded artifacts、docs 镜像；初始化测试校验 project-local schemas |
| C-04 | CLI `validate_cli_ignores_tampered_project_local_schemas` |
| V-04 / V-08 | `default-projection-contract.json` 被 TS/Rust/CLI 消费；Rust raw clone equality、CLI 文件 bytes 不变测试 |
| V-06 / V-07 | `node-semantic-contract.json` 覆盖 registry、characterExpression、registryByDiscriminator、missing/empty/duplicate story point，并比较 code/severity/source/jsonPath |
| V-10 | `validate_cli_returns_one_for_contract_invalid_graph` 与真实进程 `cli_process_contract.rs` |
| W-01 | `save_file_cannot_bypass_control_document_contracts`、invalid node、graph/manifest/meta 与 graph-position write gate 测试 |
| W-02 / M-04 | `backend_module_contract.rs` 锁定 26 个 command、注册集合与 JSON 参数 key |
| P-01 | `backend/tests/path_security.rs` 覆盖 content/control/node/asset/workspace symlink 越界 |
| P-02 | `backend/commands/tests.rs` 覆盖 A→B revoke/allow、幂等和失败补偿 |
| M-01 / M-02 / M-03 | `backend_module_contract.rs` 检查无 `include!`、领域边界、依赖方向、commands-only Tauri、public CLI façade |
| R-01 / R-02 / R-03 | `.github/workflows/ci.yml` 的 exact MSRV/offline 全测试、macOS/Windows bundle、无 checkout 安装版 CLI validate/build/browser smoke |

共享结构 corpus 还包含 66 条错误的超限 case，三端固定输出 64 条结构 issue 加一个
`contract_error_truncated`，并按 jsonPath/code 稳定排序。
...
- 2026-07-10：`@vibegal/contracts` 成为 graph/node/manifest/meta、诊断元数据和 Instruction policy 的唯一来源。
- 2026-07-10：Rust/CLI 改为嵌入并校验可追溯生成物，删除手写 Instruction 合法 tag/字段镜像和旧双轨 validator。
- 2026-07-10：所有受控 graph/manifest/meta/node 写入入口接入同一 embedded contract gate，raw defaults 不写回磁盘。
- 2026-07-10：建立 `ProjectRoot` / `ContentRoot` 路径能力并补齐 control、asset、renderer symlink 与 size/count 防护。
- 2026-07-10：backend 完成正式领域模块化，删除 `imports.rs`、普通 `include!` 和旧 monolithic tests。
- 2026-07-10：CI 增加 exact MSRV、locked/offline 全测试，以及无源码 checkout 的 macOS/Windows 安装版 CLI smoke。
