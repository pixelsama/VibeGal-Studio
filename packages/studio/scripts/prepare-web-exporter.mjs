...
export function prepareWebExporter(outDir = parseOutArg(process.argv.slice(2))) {
  ...
  const copies = [
    ...
    [path.join(repoRoot, "scripts/desktop-update-client.mjs"), "scripts/desktop-update-client.mjs"],
    [path.join(repoRoot, "scripts/verify-update-manifest.mjs"), "scripts/verify-update-manifest.mjs"],
    [path.join(repoRoot, "scripts/update-manifest-contract.mjs"), "scripts/update-manifest-contract.mjs"],
