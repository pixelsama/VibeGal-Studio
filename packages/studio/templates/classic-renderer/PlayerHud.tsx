...
export function PlayerHud({
  ...
  return (
    ...
    >
      ...
      {/* 悬停和焦点反馈走 stylesheet；激活态使用经典暖金。 */}
      <style>{`
        [data-player-action]:not(:disabled):not([aria-pressed="true"]):hover { background: ${palette.accentSoft} !important; color: ${palette.accent} !important; }
        [data-player-action]:focus-visible { outline: 2px solid ${palette.accent}; outline-offset: 2px; }
      `}</style>
    </nav>
  );
}
...
// bgColor token 作用于整条工具栏底色；激活态使用经典暖金作为状态反馈。
