...
#[cfg(target_os = "macos")]
fn webview_runtime_installed() -> bool {
  // WKWebView is part of macOS. Probing it through NSBundle crashes inside
  // CoreFoundation on macOS 15 for some non-ASCII application names.
  true
}
...
impl<T: UserEvent> Wry<T> {
  ...
  fn init(event_loop: EventLoop<Message<T>>) -> Result<Self> {
    ...
    let context = Context {
      ...
      webview_runtime_installed: webview_runtime_installed(),
    };
