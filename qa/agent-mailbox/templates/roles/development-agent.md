...
3. 对 `failed`、`stale` 或 `blocked` 结果复现问题；缺少覆盖时先添加失败测试，再完成最小修复。
4. 运行最窄相关验证，再按风险运行更广检查；提交后确认 worktree 干净。
5. 使用原 `requestId`、递增的 `attempt`、新的完整 40 位 `featureCommit` 生成 `test_request`，把临时 JSON 放在 `../exchange/runs/<requestId>/outgoing/`。
6. 用 `../exchange/runtime/codex/agent-mailbox.mjs enqueue` 原子入队给指定测试 Agent。入队成功后，才把已处理结果归档。
